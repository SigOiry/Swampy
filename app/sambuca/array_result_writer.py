...
class ArrayResultWriter(PixelResultHandler):
    ...
    **In the short-term, this class could be modified to add additional outputs,
    ...
    def __init__(
        ...
        self._reference_560_index = int(np.argmin(np.abs(self._wavelengths - 560.0)))
        ...
        self.sub1_frac = np.full((height, width), np.nan)
        self.sub2_frac = np.full((height, width), np.nan)
        self.sub3_frac = np.full((height, width), np.nan)
        ...
        self.exp_bottom = np.zeros((height, width))
        self.initial_guess_stack = None
        if store_initial_guesses:
            self.initial_guess_stack = np.full((height, width, 7), np.nan, dtype=np.float32)
    ...
    def __call__(self, x, y, observed_rrs, parameters=None, nit=None, success=None):
        ...
        kd_out = float(np.asarray(model_results.kd, dtype=float)[self._reference_560_index])

        """r_substratum = sbc.apply_sensor_filter(
            model_results.r_substratum,
            self._sensor_filter)"""
        r_sub_out = float(np.asarray(model_results.r_substratum, dtype=float)[self._reference_560_index])
      
        closed_rrs, closed_rrsdp = self._filter_spectrum(
            np.vstack((model_results.rrs, model_results.rrsdp)))
        ...
        self.exp_bottom[x,y] = model_results.exp_bottom
